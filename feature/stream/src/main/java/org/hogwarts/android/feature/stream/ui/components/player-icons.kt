...
/**
 * Player glyphs copied verbatim from the web video-overlay SVG paths
 * (`components/stream/shared/video-player/video-overlay.tsx`). Each web path
 * lives inside a viewBox with a nonzero origin — we translate the group by the
 * negative offset so coordinates line up with the (0,0)-based ImageVector
 * viewport.
 *
 * Keeping the paths byte-for-byte identical is the point: the mobile player
 * should read as the same product as the web one, not a Material lookalike.
 */

val StreamPlayIcon: ImageVector by lazy {
...
val StreamPauseIcon: ImageVector by lazy {
...
val StreamRewind10Icon: ImageVector by lazy {
...
val StreamForward10Icon: ImageVector by lazy {
    // viewBox="84.21 19.78 99.61 109.06"
    ImageVector.Builder(
        name = "StreamForward10",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 99.61f,
